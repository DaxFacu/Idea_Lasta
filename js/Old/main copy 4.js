...
const DatosSelect = () =>{
    ...
    // console.log(document.getElementById("go").value);
}
...
const buscarModeloCategoria = (categorias) => {
    if(DatosSelect!="intro"){
        guardarModelosLC(modelos);

        const productos = cargarModelosLC();
    
    guardarModelosLC(productos.filter(item => item.categoria == categorias));
    renderModelos();
    }
    else {
        guardarModelosLC(modelos);
...
document.getElementById("go").addEventListener("click", buscarModeloCategoria(DatosSelect));

// buscarModeloCategoria("anime");